	...
	public function factionChat(PlayerChatEvent $PCE) {
		...
		if($this->plugin->motdWaiting($player)) {
			if(time() - $this->plugin->getMOTDTime($player) > 30) {
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("Usa /c desc."));
				...
				$PCE->setCancelled(true);
				$PCE->getPlayer()->sendMessage($this->plugin->formatMessage("descrizione aggiornata! Utilizzare / c stats.", true));
			}
			return true;
		}
				...
				foreach($this->plugin->getServer()->getOnlinePlayers() as $fP){
					if($this->plugin->getPlayerFaction($fP->getName()) == $faction){
						if($this->plugin->getServer()->getPlayer($fP->getName())){
							$PCE->setCancelled(true);
							$this->plugin->getServer()->getPlayer($fP->getName())->sendMessage(TextFormat::DARK_GREEN."§cCHAT".TextFormat::AQUA." $player - ".TextFormat::GRAY. $msg);
						}
					}
				}
			}
			...
			if($this->plugin->allyChatActive[$player]){
				...
				foreach($this->plugin->getServer()->getOnlinePlayers() as $fP){
					if($this->plugin->areAllies($this->plugin->getPlayerFaction($fP->getName()), $faction)){
						if($this->plugin->getServer()->getPlayer($fP->getName())){
							$PCE->setCancelled(true);
							$this->plugin->getServer()->getPlayer($fP->getName())->sendMessage(TextFormat::WHITE."[$faction]".TextFormat::YELLOW." $player: ".TextFormat::GOLD. $msg);
							$PCE->getPlayer()->sendMessage(TextFormat::WHITE."[$faction]".TextFormat::YELLOW." $player: ".TextFormat::GOLD. $msg);
						}
					}
				}
			}
	...
	public function factionBlockBreakProtect(BlockBreakEvent $event) {
		if($this->plugin->isInPlot($event->getPlayer())) {
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				$event->getPlayer()->sendMessage($this->plugin->formatMessage("area dominata! usa /c plotinfo per sapere a chi appartiene."));
				return true;
			}
		}
	}
	...
	public function factionBlockPlaceProtect(BlockPlaceEvent $event) {
		if($this->plugin->isInPlot($event->getPlayer())) {
			if($this->plugin->inOwnPlot($event->getPlayer())) {
				...
				$event->setCancelled(true);
				$event->getPlayer()->sendMessage($this->plugin->formatMessage("Zona dominata usa /c plotinfo per sapere di chi è."));
				return true;
			}
		}
	}
