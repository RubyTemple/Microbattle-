...
class FactionCommands {
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args) {
		if($sender instanceof Player) {
			...
			if(strtolower($command->getName('clan'))) {
				...
				if(count($args == 2)) {
					...
					if($args[0] == "war") {
						...
						if(!$this->plugin->isLeader($player)){
							$sender->sendMessage($this->plugin->formatMessage("Solo il creatore del clan può avviare una guerra!"));
							return true;
						} 
							...
							foreach($this->plugin->war_req as $r => $f) {
								if($r == $args[1] && $f == $sFaction) {
									foreach($this->plugin->getServer()->getOnlinePlayers() as $p) {
										...
										$p->sendMessage("Il duello tra §7 $factionName §7vs§7 $sFaction §dè cominciato!");
							...
							foreach($this->plugin->getServer()->getOnlinePlayers() as $p) {
								if($this->plugin->getPlayerFaction($p->getName()) == $factionName) {
									if($this->plugin->getLeader($factionName) == $p->getName()) {
										$p->sendMessage("§e$sFaction vuole fare una guerra con il tuo clan, 'Usa /c war $sFaction' per iniziare.");
										$sender->sendMessage("Richiesta di duello inviata!");
										return true;
									}
								}
							}
							$sender->sendMessage("Il creatore del clan da te selezionato non è online.");
							return true;
						}
					}
					...
					if($args[0] == "crea") {
						if(!isset($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Usa - /clan crea <nome del clan>"));
							return true;
						}
						if(!(ctype_alnum($args[1]))) {
						...
						if($this->plugin->isNameBanned($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Questo nome non è valido."));
							return true;
						}
						if($this->plugin->factionExists($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Questo clan esiste già"));
							return true;
						}
						if(strlen($args[1]) > $this->plugin->prefs->get("MaxFactionNameLength")) {
							$sender->sendMessage($this->plugin->formatMessage("Nome clan troppo lungo."));
							return true;
						}
						if($this->plugin->isInFaction($sender->getName())) {
							...
							$sender->sendMessage($this->plugin->formatMessage("Hai creato il clan $args[1]", true));
							return true;
						}
					}
					...
					if($args[0] == "invita") {
						if(!isset($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Usa - /c invita <giocatore>"));
							return true;
						}
						...
						if($this->plugin->prefs->get("OnlyLeadersAndOfficersCanInvite")) {
							    ...
							    $sender->sendMessage($this->plugin->formatMessage("Solo il Leader può invitare i giocatori nel clan."));
							    return true;
                            } 
						}
                        if($invited->getName() == $player){
                            
				            $sender->sendMessage($this->plugin->formatMessage("Non puoi invitare te stesso"));
                            return true;
                        }
				        ...
				        $invited->sendMessage($this->plugin->formatMessage("§aIl Clan§3 $factionName §eti ha invitato, Usa '/clan accetta' o '/clan rifiuta", true));
					...
					if($args[0] == "promuovi") {
						if(!isset($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("UsA - /clan promuovi <membro>"));
							return true;
						}
						if(!$this->plugin->isInFaction($sender->getName())) {
					...
					if($args[0] == "retrocedi") {
						...
						$result = $stmt->execute();
						$player = $this->plugin->getServer()->getPlayerExact($args[1]);
						$sender->sendMessage($this->plugin->formatMessage("$args[1] §cretrocesso!", true));
						if($player instanceof Player) {
					...
					if($args[0] == "kick") {
						if(!isset($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Usa - /clan kick <membro>"));
							return true;
						}
						if($this->plugin->isInFaction($sender->getName()) == false) {
							$sender->sendMessage($this->plugin->formatMessage("Crea un clan prima!"));
                        ...
                        if($args[1] == $sender->getName()){
                            $sender->sendMessage($this->plugin->formatMessage("Non ci si può espellere da solo."));
							return true;
                        }
					...
					if(strtolower($args[0]) == 'info') {
						if(isset($args[1])) {
							...
							$sender->sendMessage(TextFormat::GOLD . TextFormat::AQUA . "|*Descrizione*| - " . TextFormat::GOLD . "§3$message" . TextFormat::RESET);
							...
							$sender->sendMessage(TextFormat::GOLD . TextFormat::AQUA . "|*Descrizione*| - " . TextFormat::BLUE . "§3$message" . TextFormat::RESET);
							$sender->sendMessage(TextFormat::GOLD . TextFormat::YELLOW . "§a§o-------§4Info del clan§a-------" . TextFormat::RESET);
						}
					}
				...
				if(count($args == 1)) {
					...
					if(strtolower($args[0]) == 'claim') {
						...
						$sender->sendMessage($this->plugin->formatMessage("§aSalvando le coordinate...", true));
                    ...
                    if(strtolower($args[0]) == 'plotinfo'){
                        ...
                        $sender->sendMessage($this->plugin->formatMessage("§7Questa terra possiede già un proprietario §5$fac. Il clan ha $power di potere"));
                    }
                    ...
                    if(strtolower($args[0]) == 'fc') {
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c fc <clan>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
                    ...
                    if(strtolower($args[0]) == 'addpw') {
                        if(!isset($args[1]) or !isset($args[2])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c addpw <clan> <PODER>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("La funzione richiesta non esiste."));
                    ...
                    if(strtolower($args[0]) == 'pf'){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c pf <player>"));
                            return true;
                        }
                    ...
                    if(strtolower($args[0]) == 'overclaim') {
						...
						if($this->plugin->getNumberOfPlayers($faction) < $this->plugin->prefs->get("PlayersNeededInFactionToClaimAPlot")){
                           ...
                           $needed_players =  $this->plugin->prefs->get("PlayersNeededInFactionToClaimAPlot") - 
                                               $this->plugin->getNumberOfPlayers($faction);
                           $sender->sendMessage($this->plugin->formatMessage("Sono necessari $needed_players giocatori per dominare questo terreno"));
				           return true;
                        }
                        if($this->plugin->getFactionPower($faction) < $this->plugin->prefs->get("PowerNeededToClaimAPlot")){
                            $needed_power = $this->plugin->prefs->get("PowerNeededToClaimAPlot");
							...
							$sender->sendMessage($this->plugin->formatMessage("$needed_power è il potere richiesto per riscvattare questo terreno, ma il tuo clan ne ha solo $faction_power."));
                            return true;
                        }
						$sender->sendMessage($this->plugin->formatMessage("§aSalvando coordinate...", true));
						$x = floor($sender->getX());
						$y = floor($sender->getY());
						$z = floor($sender->getZ());
                        if($this->plugin->prefs->get("EnableOverClaim")){
                            ...
                            $sender->sendMessage($this->plugin->formatMessage("L'Overclaiming è disabilitato."));
                            return true;
                        }
					...
					if(strtolower($args[0]) == "unclaim") {
						...
						$sender->sendMessage($this->plugin->formatMessage("Terreno abbandonato.", true));
					}
					...
					if(strtolower($args[0]) == "desc") {
						...
						$sender->sendMessage($this->plugin->formatMessage("Inserisci il tuo messaggio nella chat. Non sarà visibile ad altri giocatori", true));
					...
					if(strtolower($args[0]) == "accetta") {
						...
						$array = $result->fetchArray(SQLITE3_ASSOC);
						if(empty($array) == true) {
							$sender->sendMessage($this->plugin->formatMessage("Non hai inviti al momento!"));
							return true;
						}
						...
						if(($currentTime - $invitedTime) <= 60) { //il tempo può essere cambiato
							...
							$sender->sendMessage($this->plugin->formatMessage("Sei entrato nel CLAN§6 $faction!", true));
					...
					if(strtolower($args[0]) == "rifiuta") {
						...
						if( ($currentTime - $invitedTime) <= 60 ) { //puoi modificare il tempo
							...
							$this->plugin->getServer()->getPlayerExact($array["invitedby"])->sendMessage($this->plugin->formatMessage("$player ha rifutato l'invito."));
						} else {
							$sender->sendMessage($this->plugin->formatMessage("invito scaduto!"));
							$this->plugin->db->query("DELETE * FROM confirm WHERE player='$lowercaseName';");
						}
					...
					if(strtolower($args[0] == "esci")) {
						if($this->plugin->isLeader($player) == false) {
							...
							$sender->sendMessage($this->plugin->formatMessage("E' necessario eliminare il CLAN, o dare il ruolo di Leader a qualcun'altro."));
						}
					}
					...
					if(strtolower($args[0] == "sethome")) {
						...
						if(!$this->plugin->isLeader($player)) {
							$sender->sendMessage($this->plugin->formatMessage("Non hai i permessi per farlo."));
							return true;
						}
					...
					if(strtolower($args[0] == "unsethome")) {
						...
						$faction = $this->plugin->getPlayerFaction($sender->getName());
						$this->plugin->db->query("DELETE FROM home WHERE faction = '$faction';");
						$sender->sendMessage($this->plugin->formatMessage("Coordinate casa non trovate!", true));
					}
                    ...
                    if(strtolower($args[0] == "mof")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c mof <Clan>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Clan inesistente."));
                    ...
                    if(strtolower($args[0] == "Off")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c Off <clan>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("Clan inesistente."));
                    ...
                    if(strtolower($args[0] == "ofl")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c ofl <clan>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
                    ...
                    if(strtolower($args[0] == "say")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c say <messaggio>"));
                            return true;
                        }
                        if(!($this->plugin->isInFaction($player))){
                        ...
                        for($i=0; $resultArr = $result->fetchArray(SQLITE3_ASSOC); $i=$i+1){
                            ...
                            if($p instanceof Player){
                                $p->sendMessage(TextFormat::RED . "-F§dSay- " . TextFormat::GOLD . "[$rank$f]" . TextFormat::YELLOW . " [$player] : " . TextFormat::RESET);
                                $p->sendMessage(TextFormat::WHITE . $message . TextFormat::RESET);
					...
					if(strtolower($args[0] == "alleati")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /clan alleati <clan>"));
                            return true;
                        }
                        if(!$this->plugin->isInFaction($player)) {
							$sender->sendMessage($this->plugin->formatMessage("Devi avere un clan."));
                    ...
                    if(strtolower($args[0] == "allea")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c allea <clan>"));
                            return true;
                        }
                        if(!$this->plugin->isInFaction($player)) {
							$sender->sendMessage($this->plugin->formatMessage("Devi avere un clan."));
                        ...
                        if($this->plugin->getPlayerFaction($player) == $args[1]){
                            $sender->sendMessage($this->plugin->formatMessage("Il tuo clan ha poco potere e non può ancora ottenere alleanze."));
                            return true;
                        }
                        if($this->plugin->areAllies($this->plugin->getPlayerFaction($player),$args[1])){
                    ...
                    if(strtolower($args[0] == "unallea")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /clan unallea <clan>"));
                            return true;
                        }
                        if(!$this->plugin->isInFaction($player)) {
							$sender->sendMessage($this->plugin->formatMessage("Devi prima creare un clan."));
                        ...
                        $this->plugin->updateAllies($fac);
                        $this->plugin->updateAllies($args[1]);
                        $sender->sendMessage($this->plugin->formatMessage("Il clan§6 $fac §eha cancellato l'alleanza con§6 $args[1]!",true));
                        if($leader instanceof Player){
                            $leader->sendMessage($this->plugin->formatMessage("Ll capo del clan§6 $fac ha rotto l'alleanza con il tuo clan§6 $args[1]!",false));
                        }
                    ...
                    if(strtolower($args[0] == "domina")){
                        if(!isset($args[1])){
                            $sender->sendMessage($this->plugin->formatMessage("Usa - /c domina <clan>"));
                            return true;
                        }
                        if(!$this->plugin->factionExists($args[1])) {
							$sender->sendMessage($this->plugin->formatMessage("CLAN non trovato"));
				        ...
				        $sender->sendMessage($this->plugin->formatMessage("Terreno dominato da $args[1]!"));
                        $this->plugin->db->query("DELETE FROM plots WHERE faction='$args[1]';");
                    ...
                    if(strtolower($args[0] == "si")){
						...
						$array = $result->fetchArray(SQLITE3_ASSOC);
						if(empty($array) == true) {
							$sender->sendMessage($this->plugin->formatMessage("Il tuo clan non ha avuto richieste di alleanze!"));
							return true;
						}
					...
					if(strtolower($args[0] == 'plugin')) {
						...
						$sender->sendMessage(TextFormat::GOLD . "[MODIFICATO in] Clans v1.1.8 by " . TextFormat::BOLD . "Emis");
					}
					////////////////////////////// CHAT ////////////////////////////////
					...
					if(strtolower($args[0]) == "ac" or strtolower($args[0]) == "ac"){
						if($this->plugin->isInFaction($player)){
							if(isset($this->plugin->allyChatActive[$player])){
								...
								$sender->sendMessage($this->plugin->formatMessage("Chat con alleanze §cdisattivata!", false));
								return true;
							}
							else{
								$this->plugin->allyChatActive[$player] = 1;
								$sender->sendMessage($this->plugin->formatMessage("§aChat con alleanze §ain funzione!", false));
								return true;
							}
						}
						else{
			...
			$this->plugin->getServer()->getLogger()->info($this->plugin->formatMessage("Plugin attivo by §eEmis §fdi §aIta§fSky§4Games - usalo all'interno del gioco"));
		}
	}
